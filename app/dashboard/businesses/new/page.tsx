  ...
  useEffect(() => {
    if (formData.locationId) {
      ...
      if (location) {
        ...
        // Load city/zip data for this location
        const zipData = locationZipCodes[location.slug]
        if (zipData) {
          setCityZipData(zipData.cities)
          // If only one city, auto-select it
          if (zipData.cities.length === 1) {
            setFormData(prev => ({ ...prev, city: zipData.cities[0].city }))
          }
        } else {
          setCityZipData([])
        }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div>
              ...
              <select
                ...
                onChange={(e) => {
                  handleChange(e)
                  setFormData(prev => ({ ...prev, directoryId: '', city: '', zipCode: '' })) // Reset directory, city, zip
                }}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    handleChange(e)
                    setFormData(prev => ({ ...prev, zipCode: '' })) // Reset zip when city changes
                  }}
                ...
                {cityZipData.length === 1 && (
                  <p className="mt-1 text-xs text-slate-500">Only one city available for this location</p>
                )}
              </div>
